 ...
 */
async function generateSectionAudio(content: string, language: string): Promise<Buffer> {
...
async function uploadAudioToStorage(
  ...
  const { error } = await supabase.storage.from("glossa-modules").upload(filePath, audioBuffer, {
...
export async function generateAudioForSection(
  ...
  try {
    ...
    // Upload to storage
    const audioUrl = await uploadAudioToStorage(supabase, audioBuffer, moduleId, sectionId);
  ...
  for (const [sectionId, result] of audioResults) {
    if (result.success && result.audioUrl) {
      const updatePromise = (async () => {
        ...
        if (error) {
          console.error(`Failed to update audio_url for section ${sectionId}:`, error);
        }
      })();
