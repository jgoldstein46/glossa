  ...
  try {
    ...
    for (const answer of answers) {
      if (!answer.question_id || typeof answer.user_response !== "string") {
        return apiError("Each answer must have question_id and user_response", 400);
      }
    }
    ...
    const providedIds = answers.map((a: { question_id: string }) => a.question_id);

    const missingQuestions = questionIds.filter((id: string) => !providedIds.includes(id));

    if (missingQuestions.length > 0) {
      return apiError(`Missing answers for questions: ${missingQuestions.join(", ")}`, 400);
    }
